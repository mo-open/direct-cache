...
public class MergingByteBufferAllocator extends AbstractByteBufferAllocator {
    ...
    // Set of usedMemory pointers. The key is #getHash(ByteBuffer).
    private final Map<Integer, LinkedByteBuffer> usedPointers = new ConcurrentHashMap<Integer, LinkedByteBuffer>();

    // Lock usedMemory instead of synchronized block to guarantee consistency when manipulating list of pointers.
     ...
     */
    public MergingByteBufferAllocator(final int totalSize) {
        super();
    ...
    /**
     * Generate free sizes' range. Sizes' range are usedMemory to try to allocate the best matching {@ByteBuffer}
     * regarding the requested size. Instead of using a sorted structure, arbitrary size's range are computed.
     *
     * @param totalSize
     * @return a list of all size's level usedMemory by the allocator.
     */
