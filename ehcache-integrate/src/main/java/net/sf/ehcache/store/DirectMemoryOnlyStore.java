...
public class DirectMemoryOnlyStore extends FrontEndCacheTier<NullStore, DirectMemoryStore> {
    ...
    protected static class BruteForceSearchManager implements SearchManager {
        ...
        private final DirectMemoryStore memoryStore;
         ...
         */
        public BruteForceSearchManager(DirectMemoryStore memoryStore) {
        ...
        public Results executeQuery(String cacheName, StoreQuery query, Map<String, AttributeExtractor> extractors) {
            ...
            boolean includeResults = query.requestsKeys() || query.requestsValues()
                    || !query.requestedAttributes().isEmpty() || isGroupBy;

            boolean hasOrder = !query.getOrdering().isEmpty();

            final Map<Set<?>, BaseResult> groupByResults = new HashMap<Set<?>, BaseResult>();
            final Map<Set, List<AggregatorInstance<?>>> groupByAggregators =
                    new HashMap<Set, List<AggregatorInstance<?>>>();
            ...
            for (Element element : matches) {
                if (includeResults) {
                    final Map<String, Object> attributes = getAttributeValues(query.requestedAttributes(),
                            extractors, element);
                    final Object[] sortAttributes = getSortAttributes(query, extractors, element);

                    if (!isGroupBy) {
                        results.add(new ResultImpl(element.getObjectKey(), element.getObjectValue(), query,
                        ...
                        if (group == null) {
                             /* placeholder for now */
                            group = new GroupedResultImpl(query, attributes, sortAttributes, Collections.emptyList(),
                                    groupByValues);
            ...
            return new ResultsImpl((List) results, query.requestsKeys(), query.requestsValues(),
                    !query.requestedAttributes().isEmpty(), anyMatches
                    && !aggregators.isEmpty());
        }
        ...
        private Map<String, Object> getAttributeValues(Set<Attribute<?>> attributes,
                                                       Map<String, AttributeExtractor> extractors, Element element) {
        ...
        private void aggregate(List<AggregatorInstance<?>> aggregators, Map<String, AttributeExtractor> extractors,
                               Element element) {
            for (AggregatorInstance<?> aggregator : aggregators) {
                ...
                if (attribute == null) {
                    ...
                    Object val = extractors.get(attribute.getAttributeName()).attributeFor(element,
        ...
        private Object[] getSortAttributes(StoreQuery query, Map<String, AttributeExtractor> extractors,
                                           Element element) {
